            ...
            return webClientAutenticacion.post()
                    ...
                    .flatMap(response ->{
                        if(response.codigo().equals(00)){
                            ...
                            return Mono.just(new LogoutResponseDTO("00","cerrar sesion"));
                        }else {

                            return Mono.just(new LogoutResponseDTO("01","Error: No se puede cerrar sesion en la pagina"));
                        }
                    });
        } catch (Exception e) {
            System.out.println(e.getMessage());
            return Mono.just(new LogoutResponseDTO("99",e.getMessage()));
        }
    }
}